...
(function($){
	...
	bValidator = function(mainElement, overrideOptions){
		...
		_showMsg = function(element, messages){
			...
			msg_container = $('<class="bVErrMsgContainer">').css('position','hidden');
			element.data("errMsg.bV", msg_container);
			msg_container.insertAfter(element);
		...
		this.validate = function(doNotshowMessages, elementsOverride){
					...
					if(!is_valid){
						...
						if(!doNotshowMessages && onAfterElementValidation !== false){
							...
							if(errorMessages.length){
								...
								if(onAfterElementValidation !== 0 && $(this).val() !== "" && $(this).val() !=="MM/DD/YYYY" && $(this).val() !=="(xxx) xxx-xxxx" && $(this).val() !=="Select") {
            ...
            this.onFocusInfo = function(id) {
			errorMessages = [];
			errorMessages[errorMessages.length] = 'Field is Required';
			elements = _getElementsForValidation(mainElement);
			elements.each(function(){
				if ($(this).attr('id') == id) {
					$(this).addClass(options.errorClass);
						if ($(this).val() == "" || $(this).val() == "MM/DD/YYYY" || $(this).val() == "(xxx) xxx-xxxx" || $(this).val() == "Select"){
					_showErrMsg($(this), errorMessages)
				}
				}
			});
...
//				$(this).addClass(options.errorClass);
			...
			if(scroll_to && !elementsOverride && ($(window).scrollTop() > scroll_to || $(window).scrollTop()+$(window).height() < scroll_to)){
				...
				$(ua.indexOf('chrome')>-1 || ua.indexOf('safari')>-1 || ua.indexOf('firefox')>-1 ? 'body' : 'html').animate({scrollTop: scroll_to - 10}, {duration: 'slow'});
